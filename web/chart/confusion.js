class Confusion{
    constructor(container, samples, classes, options){
        ...
        //topText.style.width = this.cellSize + "px";
    ...
    #fillTable(){
        ...
        const values = matrix
            //remove first row
            .slice(1)
            //remove first element of each row
            .map((t) => t.slice(1))
            //Flatten 2D Array into a 3D Array 
            .flat();
        ...
        for(let i=0; i<N; i++){
            ...
            for(let j=0; j<N;j++){
                ...
                if (i > 0 && j > 0){
                    //invLerp function calculates relative part of a value between two boundaries
